...
-(void)startFire
{
    ...
    //1: Adding stone and wood image (campfire) as a base for fire particle system.
    ...
    //4: Re-using feather texture as fire particle, to demonstrate how white feathers become flames.
    fire.texture = [CCTexture textureWithFile:@"feather.png"];
    
    //5: Scaling it down a bit, since by default it is too big. Using scale property instead of tuning the PS itself.
    fire.scale = 0.3f;
    
    //6: Adding to the scene.
    [self addChild:fire];
}
...
-(void)addPauseButton
{
    ...
    [CCSpriteFrame frameWithImageNamed:@"btn_pause.png"];
    CCSpriteFrame *pauseHighlightedImage  =
    [CCSpriteFrame
     frameWithImageNamed:@"btn_pause_pressed.png"];
