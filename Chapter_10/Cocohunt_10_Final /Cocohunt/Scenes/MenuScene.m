...
-(void)addAudioButtons
{
    ...
    //2: Setting it to toggle mode
    _btnSoundToggle.togglesSelectedState = YES;
    
    //3: Getting initial value from the AudioManager
    _btnSoundToggle.selected = [AudioManager sharedAudioManager].isSoundEnabled;
    
    //4: On tap simply toggling sound
    ...
    //6: Music button is very similiar to sound button.
...
-(void)btnHighscoresTapped:(id)sender
{
    ...
    CCTransition *hsTransition = [CCTransition transitionPushWithDirection:CCTransitionDirectionDown duration:1.0f];
